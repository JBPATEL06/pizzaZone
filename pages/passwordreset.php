...
if(isset($_REQUEST['pwdrst']))
{
  ...
  if($pwd == $cpwd)
  {
    $reset_pwd = mysqli_query($conn,"update user set password='$pwd' where emailId='$emailId'");
    if($reset_pwd>0)
    {
...
if($_GET['secret'])
{
  $emailId = base64_decode($_GET['secret']);
  $check_details = mysqli_query($conn,"select emailId from user where emailId='$emailId'");
